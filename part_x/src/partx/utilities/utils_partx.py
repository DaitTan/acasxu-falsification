...
def calculate_distances(X, test_function_dimension):
    
    dimension = X.shape[1]
    num_samples = X.shape[0]
    assert dimension == test_function_dimension
    d_x = np.zeros((num_samples*num_samples,dimension))
    hh = 0
...
def initial_theta_estimate(X):
    test_function_dimension = X.shape[1]
    d_x = calculate_distances(X,test_function_dimension)
    multiplier = (np.log(2)/test_function_dimension)
    term = np.mean(np.absolute(d_x),0)
            
    initial_theta = (multiplier*term)
